...
在“词汇地图”点击 **扫描最新对话**，工作台会优先重读新增或变化的 Codex 任务；如果词表因此出现此前没有的表达，它们会以蓝色外圈标为“本次新增”。也可以点 **添加表达**，输入英文和中文，让一个自己遇到的新词立即进入个人词表与图谱。领域可留空由本地规则判断，也可手动选择。
...
当前关系推断在本机以可解释的规则运行；它不是云端大模型语义检索。历史扫描目前主要命中内置基础词表与预设领域表达，因此新对话不一定每次都会带来新词。遇到未覆盖的表达，可先手动接入；后续版本会扩展开放式短语提取。
