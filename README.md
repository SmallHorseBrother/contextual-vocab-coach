...
> 把你真实做过的事说出来，整理成属于你的英语词表，再把少量表达送进练习和复习。

`contextual-vocab-coach` 是一个可独立运行的本地英语学习工作台，也可作为 Codex Skill 使用。没有 Codex 的用户可以直接口述、粘贴或导入文字；Codex 用户还可以在本机扫描自己的任务历史。它不会要求你先想一个抽象的学习目标，也不会把每段内容割裂成一张小词表。

上下文默认隐藏，只用于排序、分类和来源追溯。用户打开后首先看到的是完整词表，而不是被迫选择“我要学哪个项目”。

## 新用户从这里开始

没有 Codex：安装 Python 3.10+，克隆仓库，在仓库根目录运行：

```powershell
git clone https://github.com/SmallHorseBrother/contextual-vocab-coach.git
cd contextual-vocab-coach
python .\contextual-vocab-coach\scripts\workbench_server.py --no-context-scan
```

macOS / Linux 将最后一行换成 `python3 ./contextual-vocab-coach/scripts/workbench_server.py --no-context-scan`。浏览器打开 `http://127.0.0.1:4174/`。第一次会看到“我的上下文”：随意写或说一段今天做过的事，也可导入 `.txt` / `.md`；点击“追加上下文并推荐词汇”。日后可用同一个名称继续追加，也可创建另一组。前端预构建产物已随仓库提供，普通使用不需要 Node.js。

有 Codex：按下文将 Skill 复制到 Codex Skills 目录，然后在 Codex 中说：

```text
$contextual-vocab-coach 打开我的词汇学习工作台
```

有 Codex 时，第一次打开会发生这些事情：

1. 扫描本机现有的活跃和已归档 Codex 任务。
2. 只把用户自己写的消息作为学习需求证据；不把助手和工具输出当作用户能力。
3. 合并 540 个基础生活表达、3200 个离线扩展词、历史中直接出现的英语，以及根据真实任务整理出的领域表达。
4. 去重后生成一份静态个人词表，并保留“词汇—主题—任务来源”的关联。
5. 打开本地工作台。用户可以直接搜索词表、加入学习、练习和复习。

首次扫描量取决于你的 Codex 历史，任务较多时可能需要几分钟。终端会显示处理进度；工作台就绪后会显示扫描到的任务数、读取的数据量和词表规模。无 Codex 模式不做任务扫描，可以立刻添加自己的内容。

后续不需要反复全量重建。Codex 用户在“我的词表”中点击 **扫描最新对话**，系统会优先处理最新任务，只重读新增或发生变化的内容。所有用户都可在“我的上下文”继续追加文字、文件或语音转写；系统会重新排序推荐，同时保留已经会了、近期不用、正在学习和复习历史。

## 这个产品到底做什么

```text
本机 Codex 任务，或你口述 / 粘贴 / 导入的文字
      ↓
真实的工作、生活与兴趣内容
      ↓
统一的个人词表（基础英语 + 高频扩展 + 上下文命中 + 领域表达）
      ↓
用户主动选择少量内容
      ↓
主动回忆 → 反馈 → 间隔复习 → 换场景迁移
...
- **词表规模可选**：500 项重点表达、1000 项常见场景、2000+（实际目标 2500）项细化表达。切换档位只改变推荐范围，不删除词，也不自动安排几千条复习。

## 工作台里有什么

- **我的上下文**：输入、浏览器语音转写或导入文字；以后随时追加，并选择推荐词表规模。
- **今日学习**：优先处理到期复习，再加入少量新表达。
- **我的词表**：搜索完整个人词表，按来源、上下文和难度做可选筛选，一键增量更新。
- **复习**：用主动回忆记录真实表现，而不是“看过答案就算会”。
- **词汇地图 / 知识图谱**：把完整词表组织成可缩放的领域网络；可搜索任意表达、筛选关系、查看为什么相连，并核对新词接入时的虚线建议。
- **来源与扫描**：核对来源、分类和（有 Codex 时的）任务覆盖情况，并暂停不想继续参与推荐的来源。
...
Codex Desktop 只是自动扫描任务与对话内调用 Skill 时需要；使用独立工作台不需要它。网页语音识别取决于浏览器是否支持及麦克风权限，不支持时可使用系统听写或文字输入。
...
使用 Codex 的用户复制完成后，重启或刷新 Codex，再使用开头的命令打开工作台。只用独立工作台的用户无需执行复制步骤。
...
浏览器打开 `http://127.0.0.1:4174/`。服务只监听本机回环地址。无 Codex 用户请添加 `--no-context-scan`；不需要安装 Codex。
...
在“词汇地图”点击 **添加上下文** 或（有 Codex 时）**扫描最新对话**，如果词表因此出现此前没有的表达，它们会以蓝色外圈标为“本次新增”。也可以点 **添加表达**，输入英文和中文，让一个自己遇到的新词立即进入个人词表与图谱。领域可留空由本地规则判断，也可手动选择。

系统会依据词义标签、表达组成、同任务出现记录或同领域/生活场景线索寻找相邻节点。高证据关系直接显示；弱关系以虚线提出，由你接受、改类型或忽略。右侧面板会说明每条关系的依据；“规则线索分”不是机器学习概率，也不能据此断定两个词是同义词。没有足够依据时，新词可以暂时独立，绝不强连。

看过新词后点 **标记这个词已看**；蓝色“新发现”状态消失，但它仍留在词表和图谱中。“已看”不等于“已掌握”，也不会自动进入复习队列。只有你主动在“我的词表”加入学习，才会排进练习。**更新前 / 更新后** 可对照最近一次真正新增的节点；“重点 / 扩展 / 全部”只改变屏幕密度，不删词。

当前上下文推荐和关系推断在本机以可解释的词义匹配、主题规则和频率排序运行；它不是云端大模型语义检索，也不能保证从任意段落创造出所有可能的短语。大档位会加入更多通用词，不代表这 2500 项都直接出现在你输入的内容中。词义是简化的学习提示，遇到多义词请在具体语境里核对。未覆盖的表达可手动接入。

## 扫描范围和隐私

默认行为是明确而激进的：有 Codex 时，正常启动真实工作台会扫描本机可用的全部活跃和已归档任务。无 Codex 用户用 `--no-context-scan` 运行；手动输入的内容在本机分析。

- 读取任务标题、时间和用户自己写过的消息。
- 不把助手回复或工具输出当作用户学习需求。
- 学习档案只保存词汇、分类、来源关联、计数、时间戳和内容指纹等派生结果。
- 不把原始聊天正文复制进学习档案。
- 手动输入或导入的文字提交后也不保存原文，只保存关联词条、主题、字符数和时间；页面会清空输入框。浏览器语音识别可能经浏览器服务处理音频，不等同于完全离线识别。
...
├── data/expanded-lexicon.json     # 3200 项高频扩展词
├── data/ECDICT-LICENSE            # 扩展词典来源许可证
├── scripts/codex_context.py       # Codex 全量与增量扫描
├── scripts/context_intake.py      # 手动上下文导入和词表档位
├── scripts/knowledge_graph.py      # 增量词汇关系与新词状态
...
python -m unittest discover -s .\contextual-vocab-coach\tests -v
...
扩展的 3200 个英汉词条选自 [ECDICT](https://github.com/skywind3000/ECDICT)，依据其 MIT 许可随项目分发，许可见 [ECDICT-LICENSE](contextual-vocab-coach/data/ECDICT-LICENSE)。生成脚本为 `contextual-vocab-coach/scripts/build_expanded_lexicon.py`；不提交上游完整 CSV。此词库仅用于离线候选推荐，词频、难度和简化词义不能替代权威词典或考试分级。
